...
| 36 | A lookup column sorts/groups by its **display path** (`Store/Emblem`), so a lookup whose display member is itself a reference/blob/collection is unsortable server-side — and projecting only `ObjectType`/`KeyMember`/`DisplayMember` leaves the client no way to know. Common: XAF's `[XafDefaultProperty]` often points at a reference | Project the display member's **data type** too, and refuse the sort up front (`DxGridDataColumn.AllowSort=false` in server mode — it is `bool?`, column overrides grid); apply the same ceiling to grouping. Treat an *absent* type as "unknown → still sortable" so an older host keeps working, and keep the strip-shaping recovery (gotcha 32) as the backstop: `AllowSort=false` blocks the header click, but `SortIndex`/`SortBy` still sort in code |
